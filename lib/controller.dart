  ...
  checkWord(){
    ...
    if(guessedWord == correctWord){
      ...
      for(int i = 0; i<remainingCorrect.length; i++){
        ...
        for(int j = 0; j<5; j++){
          if(remainingCorrect[i]==tilesEnterad[j + (currentRow * 5)].letter){
            if(tilesEnterad[j + (currentRow * 5)].answerStage != AnswerStage.correct){
              tilesEnterad[j + (currentRow * 5)].answerStage = AnswerStage.contains;
            }
